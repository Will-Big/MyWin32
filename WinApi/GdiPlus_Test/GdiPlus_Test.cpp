// GdiPlus_Test.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
#include <Windows.h>
...
{
    Graphics G(hdc);
    Bitmap image(L"Mario.bmp");
    Status s = image.GetLastStatus();
    image.GetHeight();
    image.GetWidth();

    PointF destinationPoints[] = {
       PointF(200.f, 20.f),   // destination for upper-left point of original
       PointF(110.f, 100.f),  // destination for upper-right point of original
       PointF(250.f, 30.f) };  // destination for lower-left point of original

    G.DrawImage(&image, destinationPoints, 3);
